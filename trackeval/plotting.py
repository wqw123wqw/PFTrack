...
def plot_compare_trackers(tracker_folder, tracker_list, cls, output_folder, plots_list=None):
    """Create plots which compare metrics across different predect_mot."""
    # Define what to plot
...
def load_multiple_tracker_summaries(tracker_folder, tracker_list, cls):
    """Loads summary data for multiple predect_mot."""
    data = {}
...
def create_comparison_plot(data, out_loc, y_label, x_label, sort_label, bg_label=None, bg_function=None, settings=None):
    """ Creates a scatter plot comparing multiple predect_mot between two metric fields, with one on the x-axis and the
    ...
    Inputs:
        ...
        sort_label: the metric_field_name by which predect_mot are ordered and ranked
        ...
        settings: dict of plot settings with keys:
            ...
            'num_to_plot': maximum number of predect_mot to plot
    """
    ...
    print('\nPlotting %s vs %s, for the following (ordered) predect_mot:' % (y_label, x_label))
