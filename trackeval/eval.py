...
class Evaluator:
    ...
    @_timing.time
    def evaluate(self, dataset, metrics_list, show_progressbar=False):
        """Evaluate a set of metrics on a set of datasets"""
        config = self.config
        metrics_list = metrics_list + [Count()]  # Count metrics are always run
        metric_names = utils.validate_metrics_list(metrics_list)
        dataset_name = dataset.get_name()   # 获得处理数据类的名称
        output_res = {}
        output_msg = {}
        ...
        seq_list=dataset.seq_list  # seq_list表示测试列表名字
        class_list=dataset.class_list  # class_list表示测试的类别名称

        print('\n tracker(s) on %i sequence(s) for %i class(es) on %s dataset using the following '
        ...
        try:
            ...
            seq_list_sorted = sorted(seq_list)  # 遍历列表，分别计算
            for curr_seq in tqdm.tqdm(seq_list_sorted):
                res[curr_seq] = eval_sequence(curr_seq, dataset, class_list, metrics_list,
                                              metric_names)
            ...
            output_fol = dataset.get_output_fol()
            tracker_display_name = dataset.benchmark#dataset.get_display_name(tracker)
...
def eval_sequence(seq, dataset, class_list, metrics_list, metric_names):
    ...
    #
    raw_data = dataset.get_raw_seq_data( seq)
    seq_res = {}
