...
class Detector_RGBT(object):
  ...
  def run(self, image_or_path_or_tensor, meta={}):
    ...
    if isinstance(image_or_path_or_tensor, np.ndarray):
      ...
      vi_image = image_or_path_or_tensor['vi_image'][0].numpy()
      ir_image = image_or_path_or_tensor['ir_image'][0].numpy()
    ...
    for scale in self.opt.test_scales:
      ...
      if not pre_processed:
        ...
        # prefetch testing
        vi_images = pre_processed_images['vi_images'][scale][0]
      ...
      vi_images = vi_images.to(self.opt.device, non_blocking=self.opt.non_block_test)
      ir_images = ir_images.to(self.opt.device, non_blocking=self.opt.non_block_test)

      # initializing tracker
      pre_hms, pre_inds = None, None
      if self.opt.tracking:
        # initialize the first frame
        if self.pre_vi_images is None and self.pre_ir_images is None:
          print('Initialize tracking!')
          self.pre_vi_images = vi_images
          self.pre_ir_images = ir_images
      ...
      output, dets, forward_time = self.process(
        vi_images, ir_images, self.pre_vi_images,self.pre_ir_images, pre_hms, pre_inds, return_time=True)
  ...
  def _transform_scale(self, vi_image,ir_image, scale=1):
    '''
    ...
    '''
    height, width = vi_image.shape[0:2]
    ...
    if self.opt.fix_short > 0:
      ...
      c = np.array([new_width // 2, new_height // 2], dtype=np.float32)
      s = np.array([inp_width, inp_height], dtype=np.float32)
    resized_vi_image = cv2.resize(vi_image, (new_width, new_height))
    resized_ir_image = cv2.resize(ir_image, (new_width, new_height))
    return resized_vi_image, resized_ir_image, c, s, inp_width, inp_height, height, width


  def pre_process(self, vi_image, ir_image, scale, input_meta={}):
    '''
    Crop, resize, and normalize image. Gather meta data for post processing 
      and tracking.
    '''
    resized_vi_image, resized_ir_image, c, s, inp_width, inp_height, height, width = \
      self._transform_scale(vi_image, ir_image)
    trans_input = get_affine_transform(c, s, 0, [inp_width, inp_height])
    out_height =  inp_height // self.opt.down_ratio
    out_width =  inp_width // self.opt.down_ratio
    trans_output = get_affine_transform(c, s, 0, [out_width, out_height])

    inp_vi_image = cv2.warpAffine(
      resized_vi_image, trans_input, (inp_width, inp_height),
      flags=cv2.INTER_LINEAR)
    inp_vi_image = ((inp_vi_image / 255. - self.mean) / self.std).astype(np.float32)

    inp_ir_image = cv2.warpAffine(
      resized_ir_image, trans_input, (inp_width, inp_height),
      flags=cv2.INTER_LINEAR)
    inp_ir_image = ((inp_ir_image / 255. - self.mean) / self.std).astype(np.float32)

    vi_images = inp_vi_image.transpose(2, 0, 1).reshape(1, 3, inp_height, inp_width)
    ir_images = inp_ir_image.transpose(2, 0, 1).reshape(1, 3, inp_height, inp_width)
    if self.opt.flip_test:
      vi_images = np.concatenate((vi_images, vi_images[:, :, :, ::-1]), axis=0)
      ir_images = np.concatenate((ir_images, ir_images[:, :, :, ::-1]), axis=0)
    vi_images = torch.from_numpy(vi_images)
    ...
    return vi_images,ir_images, meta
  ...
  def process(self, vi_images,ir_images, pre_vi_images=None, pre_ir_images=None, pre_hms=None,
    pre_inds=None, return_time=False):
    with torch.no_grad():
      torch.cuda.synchronize()
      output = self.model(vi_images, ir_images,  pre_vi_images, pre_ir_images, pre_hms)[-1]
  ...
  def reset_tracking(self):
    self.tracker.reset()
    self.pre_vi_images = None
    self.pre_ir_images = None
    self.pre_image_ori = None
