...
class Transformer_Fusion(nn.Module):
    def __init__(self, d_model=64, nhead=8, num_fusion_encoder_layers=2, dim_feedforward=2048,
        ...
        encoder_layer_f = TransformerEncoderLayer_f(d_model, nhead, dim_feedforward, dropout, activation, normalize_before)
        self.encoder_f = TransformerEncoder_f(encoder_layer_f, num_fusion_encoder_layers, encoder_norm)
    ...
    def forward(self, src_v,src_t, pos_embed):
        ...
        output = self.encoder_f(output_v, output_t, pos=pos_embed)
        return output
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src_v, src_t, pos=None,c=1,n=2):
        ...
        src_f = self.dropout2(src2)+src_v+src_t
        src_f = self.norm2(src_f)


        q_v = self.with_pos_embed(src_v, pos)
        k_f=self.with_pos_embed(src_f, pos)
        src_v1 = self.multihead_attn_v(q_v, k_f, value=src_f)[0]
        ...
        src_t1= self.multihead_attn_t(q_t, k_f, value=src_f)[0]
...
class TransformerEncoder_f(nn.Module):
    def __init__(self, encoder_layer, num_layers, norm=None):
        super().__init__()
        self.layers = _get_clones(encoder_layer, num_layers)
        self.norm = norm
    def forward(self, src_v, src_t, pos=None):
        ...
        output_t = src_t
        for layer in self.layers:
            output= layer(output_v,output_t, pos=pos)
        return output

        
class TransformerEncoderLayer_f(nn.Module):
