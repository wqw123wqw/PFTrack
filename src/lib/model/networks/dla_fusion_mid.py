...
class DLASeg(BaseModel):
    ...
    def imgpre2feats(self, x_rgb, x_t, pre_rgb_img=None,pre_t_img=None, pre_hm=None):
        x1 = self.base(x_rgb, pre_rgb_img, pre_hm)
        x2 = self.base(x_t, pre_t_img, pre_hm)
        

        x = []
        for vi_tensor , ir_tensor in  zip (x1, x2):
            fuse_tensor = vi_tensor + ir_tensor
            x.append(fuse_tensor)
