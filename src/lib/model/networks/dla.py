...
from .transformer_fusion_v1_refine import Transformer_Fusion
...
class DLA(nn.Module):
    ...
    def forward(self, x_rgb, x_t, pre_img_rgb=None, pre_img_t=None, pre_hm=None):
        ...
        if pre_hm is not None:
            x_rgb = x_rgb + self.pre_hm_layer(pre_hm)
            x_t = x_t + self.pre_hm_layer(pre_hm)
        [B,C,H,W]=x_rgb.shape            
        ...
        result=self.fusion(x_rgb_token,x_t_token,pos_embed_token)
