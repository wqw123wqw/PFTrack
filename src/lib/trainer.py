...
class ModleWithLoss(torch.nn.Module):
  ...
  def forward(self, batch):
    pre_vi_img = batch['pre_vi_img'] if 'pre_vi_img' in batch else None
    pre_ir_img = batch['pre_ir_img'] if 'pre_ir_img' in batch else None
    pre_hm = batch['pre_hm'] if 'pre_hm' in batch else None
    outputs = self.model(batch['vi_image'],batch['ir_image'], pre_vi_img, pre_ir_img, pre_hm)
...
class Trainer(object):
  ...
  def run_epoch(self, phase, epoch, data_loader):
    ...
    for iter_id, batch in enumerate(data_loader):
      ...
      print('{phase}: [{0}][{1}/{2}]|Tot: {total:} |ETA: {eta:} '.format(
        epoch, iter_id, num_iters, phase=phase,
        total=bar.elapsed_td, eta=bar.eta_td))
      for l in avg_loss_stats:
        avg_loss_stats[l].update(
          loss_stats[l].mean().item(), batch['vi_image'].size(0))
  ...
  def debug(self, batch, output, iter_id, dataset):
    ...
    for i in range(1):
      ...
      img = batch['vi_image'][i].detach().cpu().numpy().transpose(1, 2, 0)
      img = np.clip(((
      ...
      if 'pre_img' in batch:
        pre_img = batch['pre_vi_img'][i].detach().cpu().numpy().transpose(1, 2, 0)
        pre_img = np.clip(((
