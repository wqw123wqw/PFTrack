...
from detector_rgbt import Detector_RGBT
...
class PrefetchDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    vi_img_path = os.path.join(self.img_dir, img_info['vi_file_name'])
    ir_img_path = os.path.join(self.img_dir, img_info['ir_file_name'])
    vi_image = cv2.imread(vi_img_path)
    ir_image = cv2.imread(ir_img_path)
    vi_images, ir_images, meta = {}, {}, {}
    for scale in opt.test_scales:
      input_meta = {}
      calib = img_info['calib'] if 'calib' in img_info \
        else self.get_default_calib(vi_image.shape[1], vi_image.shape[0])
      input_meta['calib'] = calib
      vi_images[scale], ir_images[scale], meta[scale] = self.pre_process_func(
        vi_image,ir_image, scale, input_meta)
    ret = {'vi_images': vi_images, 'ir_images': ir_images, 'vi_image': vi_image, 'ir_image': ir_image, 'meta': meta}
...
def prefetch_test(opt):
  ...
  if opt.use_loaded_results:#Flase
